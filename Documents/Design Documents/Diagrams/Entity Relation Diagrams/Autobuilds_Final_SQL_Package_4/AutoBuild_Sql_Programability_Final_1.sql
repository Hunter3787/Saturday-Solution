...
USE DB
 ...
 -- 1) AUTOBUILD FUNCTIONS:
		 ...
		 -- THIS FUNCTIONS IS USED BY THE LOGINSERT PROCEDURE:
		 GO
			CREATE FUNCTION IsSessionExist
			(@SESSIONSID BIGINT)
			RETURNS INT AS
			BEGIN
			RETURN(  
			SELECT COUNT(*) FROM UserSessions US 
			WHERE US.visitorID = @SESSIONSID)
			END
