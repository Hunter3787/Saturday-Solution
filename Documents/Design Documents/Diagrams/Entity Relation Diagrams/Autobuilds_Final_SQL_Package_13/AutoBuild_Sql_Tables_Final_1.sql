...
CONSTRAINT UserAccount_Vendor_PK PRIMARY KEY(userHashID, vendorID),
...
('Create' , 'Update', 'Delete', 'Block','None','ReadOnly', 'Edit', 'All','Write', 'Save')),
  
  scopeOfPermission varchar(20) NOT NULL CHECK (scopeOfPermission IN
('None','All', 'AutoBuild', 'Self', 'Reviews', 'SelfReviews', 'Products', 'VendorProducts', 'PostBuilds',
 'Analytics', 'AdminDashBard', 'AllUsers', 'Registration', 'Login')),
  
  CONSTRAINT userPermissions_PK PRIMARY KEY(userHashID, permission,scopeOfPermission),
  CONSTRAINT userPermissions_FK FOREIGN KEY(userHashID) REFERENCES userCredentials(userHashID)   );
...
creationDate DATETIME, --NOT NULL, -- need for uad
...
CONSTRAINT	logs_HashMapping FOREIGN KEY (userHashID) 
...
ADD imagePath VARCHAR(MAX);
 ...
 userHashID BINARY(32), -- NOT NULL, chenge later ALSO CHANGE WITH THE PROCEDURE TOO (note!!!!!!!!)
 ...
 CONSTRAINT Mostpopularbuilds_CK UNIQUE (Title));
...
(userID) REFERENCES UserAccounts (userID) ON DELETE CASCADE
...
-------------------------Saving to a build             ---------------------------
 ...
 (buildID ) REFERENCES PcBuilds(buildID)  ON DELETE CASCADE,
 ...
 CONSTRAINT Save_Product_Shelf_UK UNIQUE(shelfID, itemIndex),

 CONSTRAINT Save_Shelf_FK FOREIGN KEY
 (shelfID ) REFERENCES Shelves(shelfID) ON DELETE CASCADE,
...
--------------------------------------mostpopularbuilds products reference ---------------

 CREATE TABLE Save_Product_MPB(

 postID   BIGINT NOT NULL,
 productID BIGINT NOT NULL,
 quantity  INT NOT NULL,

 CONSTRAINT Save_Product_MPB_PK PRIMARY KEY( postID  , productID),

 CONSTRAINT Save_MPB_FK FOREIGN KEY
 ( postID ) REFERENCES Mostpopularbuilds(postID) ON DELETE CASCADE,
  CONSTRAINT Product_MPB_FK FOREIGN KEY
 (productID) REFERENCES Products(productID)  ON DELETE CASCADE );


------------------------------------- end of mostpopularbuilds products reference ----------
...
CONSTRAINT reviews_PK PRIMARY KEY(entityId),
 ...
 --------------------DANNIES EMAILLIST --------------------
  ...
  productID BIGINT NOT NULL,
  CONSTRAINT email_list_pk PRIMARY KEY(email, productID),
  CONSTRAINT email_FK FOREIGN KEY(email) REFERENCES useraccounts(email),
...
---------------------- -------------------------------PRODUCTS STUFF DO NOT UNCOMMENT DO NOT TOUCH-----------------------------------------
