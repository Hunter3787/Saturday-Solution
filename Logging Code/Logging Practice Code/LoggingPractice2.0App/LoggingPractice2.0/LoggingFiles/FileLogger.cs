...
namespace LoggingPractice2._0.LoggingFiles
{
    ...
    {
        SqlConnection con = new SqlConnection(@"Data Source = Enter here; Initial Catalog = use _____; Integrated Security=True;");
        SqlCommand cmd; 
        ...
        public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter) // This allows us to customize the output of our logs, any output changes will be made here!!
        {
            ...
            cmd = new SqlCommand("insert into record values('" + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + logLevel.ToString() + "', '" + formatter(state, exception) + "', '" + (exception != null ? exception.StackTrace : "") + "','" + LogSet.GetLocalIPAddress() + "', '" + LogSet.getSessionID() + "')");
            cmd.ExecuteNonQuery();
            con.Close();
