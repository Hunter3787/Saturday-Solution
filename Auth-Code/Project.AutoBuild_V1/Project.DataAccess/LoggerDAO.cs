            ...
            using (SqlConnection conn = new SqlConnection(this.connection))
            {
                ...
                using (SqlTransaction transaction = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            CRLog.failureBool = true;
                            CRLog.Failure = $"UnSuccessful Log creation, rows affected {numberOfRowsAffected}";
                            result += $"UnSuccessful Log creation, rows affected {numberOfRowsAffected}";

                        }

                        CRLog.successBool = true;
                        CRLog.Success = $"1. Successful Log creation, rows affected {numberOfRowsAffected} \n";
                    ...
                    catch (SqlException ex)
                    {
                        if (ex.Number == -2)
                        {
                            ...
                            CRLog.failureBool = true;
                            CRLog.Failure = $"Data store has timed out.";
